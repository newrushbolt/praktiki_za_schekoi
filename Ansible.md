...
Шаблоны (templates) работают на jinja2 и поддерживают весь функционал джинджи плюс специфичные для Ansible  
  ...
  подвержен скатыванию в нечитаемый ад.
...
* Исходная структура в процессе ренедринга будет преобразована в питоновский словарь и проитерирована в  
...
Метаинформация о роли распологается в `role/meta/main.yml` и содержит информацию об авторе,  
...
* К перемнным применимы все требования именования и расположения, описанные выше.
* Дополниетльные сущности (_файлы, шаблоны, фильтры, модули_) должны находиится внутри директории  
  роли.
...
В таком случа логичными будет сделать отдельную роль, которая занимается управлением пользователями, то  
  есть создаёт нужные группы и пользователей с различными настройками (_homedir, shell, ssh keys_) и  
  удаляет пользователей тем или иным способом. И отдельну роль, которая добавляет нужных пользователей  
  в sudoers с определёнными привилегиями и удаляет их.

Это позволит использовать кажду роль, как "сервис" по управлению пользователями и настройке привилегий  
  независимо друг от друга.

#### Требования к роли

Теперь о конкретных требованиях к роли как абстракции исходя из всего написанного выше:

* **Роль не должна иметь внешних зависимостей (изолированность).**  
    Все необходимые переменные определены в _defaults_, внутри роли нет инклудов других ролей, все нужные  
    шаблоны и файлы находятся в директории роли.

* **Роль должна быть идемпотента.**  
...
* **Роль должна корректно работать в check mode.**  
    Запуск роли в check mode не должен завершаться падением выполнения роли или внесением реальных изменений.
...
только если дохуя в себе уверен, но даже тогда вероятнее всего обосрёшься
