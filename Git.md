...
* Откроется редактор  с редактированием списка коммитов  
    _Редактор может быть разным, зависит от переменной окружений EDITOR. Точно нормально работает с Vim/nano/emacs/mcedit._  
    * верхний коммит не трогать нельзя  
...
* **Внимательно** проверяем ветку, на которой находимся: это должна быть исходная ветка, и ни в коем случае **НЕ МАСТЕР**  
...
* Запускаем `git push --force`, и после этого проверяем в веб-версии, что все изменилось так, как мы хотели  
